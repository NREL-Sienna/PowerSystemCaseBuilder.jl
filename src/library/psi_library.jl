...
wind_generators = PSY.collect(get_components(gen -> gen.prime_mover_type == PrimeMovers.WT, RenewableDispatch, sys))
for i in 1:length(wind_generators)
    add_time_series!(sys, wind_generators[i], wind_DA_TS[i] )
end
solar_generators = PSY.collect(get_components(gen -> gen.prime_mover_type == PrimeMovers.PVe, RenewableDispatch, sys))
